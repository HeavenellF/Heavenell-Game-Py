...
def mainMenu_elements(width, height, font1, font3, button_surf1):
    ...
    gameTitle_rect = gameTitle_surf.get_rect(midtop=(width / 2, 40))

    button_rectPlay = button_surf1.get_rect(midtop=(width / 2, height / 2 - 90))
    button_rectSetting = button_surf1.get_rect(midtop=(width / 2, height / 2))
    button_rectExit = button_surf1.get_rect(midtop=(width / 2, height / 2 + 90))
...
def gameOver_elements(width, height, font1, font3, button_surf1):
    ...
    gameOver_rect = gameOver_surf.get_rect(midtop=(width / 2, 50))

    button_rectPlayAgain = button_surf1.get_rect(midtop=(width / 2, height / 2 - 90))
    button_rectMainMenu = button_surf1.get_rect(midtop=(width / 2, height / 2))
    button_rectExit = button_surf1.get_rect(midtop=(width / 2, height / 2 + 90))

    buttonPlayAgain_surf = font3.render('Play Again', True, 'Black')
    buttonPlayAgain_rect = buttonPlayAgain_surf.get_rect(midtop=(width / 2, height / 2 - 85))

    buttonMainMenu_surf = font3.render('Main Menu', True, 'Black')
    buttonMainMenu_rect = buttonMainMenu_surf.get_rect(midtop=(width / 2, height / 2 + 5))
